...
router.post('/send', authRequired, async (req, res, next) => 
{
  try 
  {
    ...
    const msg = await Message.create({ conv, book, from: req.user.id, to, text, readBy: [req.user.id] });

    await Notification.create({ to, type: 'message', title: 'New message', text, meta: { from: req.user.id, book } });
